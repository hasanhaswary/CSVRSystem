...
public class AuthManager : MonoBehaviour
{
    ...
    public GameObject confirmLoginText;

    //Register variables - further development
    [Header("Register")]
    ...
    private IEnumerator Login(string _email, string _password)
    {
        ...
        {
            // User is logged in
            AuthResult authResult = LoginTask.Result; // Get the AuthResult
            User = authResult.User; // Extract the FirebaseUser from AuthResult
            Debug.LogFormat("User signed in successfully: {0} ({1})", User.DisplayName, User.Email);
            warningLoginText.text = "";
            confirmLoginText.SetActive(true);
